...
export function calculateUnitEconomics(input: CalculatorInput, model: ResultMode, isAdvanced: boolean): UnitEconomicsResult {
  ...
  const adRate = input.includeAds ? clamp(input.adRate * 100, 0, 100) / 100 : 0;
  ...
  const fulfillment = input.includeFulfillment ? clamp(isAdvanced ? input.manualFulfillmentCost : defaults.fulfillmentCost, 0, 100000) : 0;
  ...
  const tax = input.includeTaxes ? calculateTax(input.taxMode, input.taxRate, salePrice, profitBeforeTax) : 0;
