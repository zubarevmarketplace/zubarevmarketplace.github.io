...
export function getCalculationContext(input: CalculatorInput, isAdvanced: boolean): CalculationContext {
  ...
  const variant1 = Math.floor(palletConfig.lengthCm / input.lengthCm) * Math.floor(palletConfig.widthCm / input.widthCm);
  const variant2 = Math.floor(palletConfig.lengthCm / input.widthCm) * Math.floor(palletConfig.widthCm / input.lengthCm);
  ...
  const groupCoefficients = {
    kolPodolskMin: Math.min(warehouseCoefficients.koledino, warehouseCoefficients.podolsk),
    krasnodarNevinnomysskMin: Math.min(warehouseCoefficients.krasnodar, warehouseCoefficients.nevinnomyssk),
  };
  ...
  const geoWarehouseCoefficient = preset.warehouseWeights.reduce((sum, item) => {
    ...
    if (item.warehouse) return sum + item.weight * warehouseCoefficients[item.warehouse];
    return sum;
  }, 0);
...
function calculateTax(taxMode: TaxMode, salePrice: number, profitBeforeTax: number, customTaxRate: number) {
...
export function calculateUnitEconomics(input: CalculatorInput, model: ResultMode, isAdvanced: boolean): UnitEconomicsResult {
  ...
  const storageDays = isAdvanced ? input.manualStorageDays : defaults.storageDays;
  const storage = model === 'fbo' ? context.volumeLiters * 0.08 * context.geoWarehouseCoefficient * storageDays : 0;
  const processing = model === 'fbs' ? defaults.fbsProcessingCost : 0;
  const adCost = input.salePrice * adRate;
  const fulfillment = input.includeFulfillment ? (isAdvanced ? input.manualFulfillmentCost : defaults.fulfillmentCost) : 0;
  ...
  const profitBeforeTax = input.salePrice - input.costPrice - commission - logistics - storage - processing - adCost - fulfillment - deliveryToWb - packaging - otherCosts;
  const tax = input.includeTaxes ? calculateTax(input.taxMode, input.salePrice, profitBeforeTax, input.customTaxRate) : 0;
  ...
  const variableWithoutTax = input.costPrice + commission + logistics + storage + processing + adCost + fulfillment + deliveryToWb + packaging + otherCosts;
  const breakEvenPrice = input.includeTaxes && (input.taxMode === 'usn6' || input.taxMode === 'custom')
    ? variableWithoutTax / (1 - (input.taxMode === 'custom' ? input.customTaxRate : 0.06))
    : variableWithoutTax;
