import rawCommissionTariffs from '../data/commissionTariffs.json';
import { geoPresets } from "../config/geoPresets";
import { defaults, palletConfig } from "../config/tariffs";
import { warehouseTariffs } from '../config/warehouseTariffs';
import type {
  CalculationContext,
  CalculatorInput,
  CommissionBreakdown,
...
const selectedCommissionTariff =
  commissionTariffs.find((t: CommissionTariff) => t.id === input.commissionTariffId) ??
  defaultCommissionTariff;

const fboCommissionRate = isAdvanced
  ? selectedCommissionTariff.fboCommissionRate
  : simpleCategory.fboCommissionRate;

const fbsCommissionRate = isAdvanced
  ? selectedCommissionTariff.fbsCommissionRate
  : simpleCategory.fbsCommissionRate;

const commissionRatePercent =
  model === 'fbo' ? fboCommissionRate : fbsCommissionRate;

const clamp = (v: number, min: number, max: number) =>
  Math.min(max, Math.max(min, Number.isFinite(v) ? v : min));

export const calculateVolumeLiters = (
  lengthCm: number,
  widthCm: number,
  heightCm: number,
...
export function calculatePalletCapacity(
  length: number,
  width: number,
  height: number,
) {
  const variant1 =
    Math.floor(palletConfig.lengthCm / length) *
    Math.floor(palletConfig.widthCm / width);
  const variant2 =
    Math.floor(palletConfig.lengthCm / width) *
    Math.floor(palletConfig.widthCm / length);
  return Math.max(
    1,
    Math.max(variant1, variant2) *
      Math.floor(palletConfig.maxHeightCm / height),
  );
}

export function detectCargoType(
  lengthCm: number,
  widthCm: number,
  heightCm: number,
  weightKg: number,
...
export function getCalculationContext(
  ...
  const groupLogistics = {
    kolPodolskMin: Math.min(
      warehouseTariffs.koledino.logistics,
      warehouseTariffs.podolsk.logistics,
    ),
  ...
  const groupStorage = {
    kolPodolskMin: Math.min(
      warehouseTariffs.koledino.storage,
      warehouseTariffs.podolsk.storage,
    ),
    krasnodarNevinnomysskMin: Math.min(
  ...
  const geoWarehouseCoefficient = preset.warehouseWeights.reduce(
    (sum, item) =>
      ...
      item.weight *
        (item.group
          ? groupLogistics[item.group]
          : item.warehouse
            ? warehouseTariffs[item.warehouse].logistics
            : 0),
    0,
  );
  const geoStorageCoefficient = preset.warehouseWeights.reduce(
    (sum, item) =>
      sum +
      item.weight *
        (item.group
          ? groupStorage[item.group]
          : item.warehouse
            ? warehouseTariffs[item.warehouse].storage
            : 0),
    0,
  );
  ...
  baseLogistics,
  itemsPerPallet,
  geoWarehouseCoefficient,
  geoStorageCoefficient,
  deliveryToWbPerUnit,
};
}
...
function calculateCommissionAndAcquiring(
  ...
  const grossWbCommission =
    priceBeforeWbDiscount * (commissionRatePercent / 100);
...
function calculateReverseLogistics(
  volumeLiters: number,
  ...
  const reverseLogisticsPerSoldUnit =
    buyout > 0 ? reverseLogisticsBase * (nonBuyout / buyout) : 0;
...
export function calculateUnitEconomics(
  ...
  const selectedCommissionTariff =
    commissionTariffs.find((t) => t.id === input.categoryId) ??
    defaultCommissionTariff;
  const buyerPrice = clamp(input.salePrice, 1, 1_000_000);
  const platformDiscountRatePercent = clamp(
    input.platformDiscountRatePercent,
    0,
    95,
  );
  const priceBeforeWbDiscount =
    buyerPrice / (1 - platformDiscountRatePercent / 100);
  ...
  function getDefaultBuyoutRatePercent(category: string, subject: string) {
  const text = `${category} ${subject}`.toLowerCase();

  if (
    text.includes('одежда') ||
    text.includes('обувь') ||
    text.includes('белье') ||
  ...
  return 90;
}const buyoutRatePercent =
  isAdvanced
    ? clamp(input.manualBuyoutRate, 1, 100)
    : getDefaultBuyoutRatePercent(
        selectedCommissionTariff.category,
        selectedCommissionTariff.subject,
      );
  const drrPercent = isAdvanced
    ? clamp(input.adRate, 0, 100)
    : input.includeAds
      ? clamp(input.adRate, 0, 100)
      : 0;
  const adCost = priceBeforeWbDiscount * (drrPercent / 100);
  const fboCommissionRate =
  isAdvanced
    ? clamp(input.manualFboCommissionRate, 0, 100)
    : clamp(selectedCommissionTariff.fboCommissionRate, 0, 100);

const fbsCommissionRate =
  isAdvanced
    ? clamp(input.manualFbsCommissionRate, 0, 100)
    : clamp(selectedCommissionTariff.fbsCommissionRate, 0, 100);
...
const warehouseCoefficientExtra =
  model === 'fbo'
    ? context.baseLogistics * context.geoWarehouseCoefficient - context.baseLogistics
    : 0;

const localizationExtra = 0;
const salesDistributionCost = 0;

const forwardLogistics =
  model === 'fbo'
    ? forwardLogisticsBase + warehouseCoefficientExtra
    : context.baseLogistics;
      : 0;
  const localizationExtra =
    model === "fbo"
      ? context.baseLogistics *
        context.geoWarehouseCoefficient *
        (localizationIndexMultiplier - 1)
      : 0;
  const salesDistributionCost =
    model === "fbo" ? priceBeforeWbDiscount * salesDistributionIndexRate : 0;
  const forwardLogistics =
    model === "fbo"
      ? forwardLogisticsBase +
        warehouseCoefficientExtra +
        localizationExtra +
        salesDistributionCost
      : context.baseLogistics;

  const rev = calculateReverseLogistics(
    context.volumeLiters,
    buyoutRatePercent,
  );
  const logistics = forwardLogistics + rev.reverseLogisticsPerSoldUnit;

  const storageDays = clamp(
  isAdvanced ? input.manualStorageDays : defaults.storageDays,
  0,
  365,
);
...
const storage =
  model === 'fbo'
    ? context.volumeLiters * 0.08 * context.geoStorageCoefficient * storageDays
    : 0;
  ...
  return {
    ...
    totalExpenses,
    isSgt: detectCargoType(
      input.lengthCm,
      input.widthCm,
      input.heightCm,
      clamp(input.weightKg, 0, 500),
    ),
