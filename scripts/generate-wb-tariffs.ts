...
const commissionOut = path.join(root, 'src/calculator/data/commissionTariffs.json');
const warehouseOut = path.join(root, 'src/calculator/config/warehouseTariffs.ts');

const slugify = (v: string) => v.toLowerCase().replace(/[^a-zа-я0-9]+/gi, '-').replace(/^-+|-+$/g, '');
const num = (v: unknown) => {
...
if (!fs.existsSync(commissionPath)) throw new Error(`Missing file: ${commissionPath}`);
if (!fs.existsSync(warehousePath)) throw new Error(`Missing file: ${warehousePath}`);

const commissionWb = xlsx.readFile(commissionPath);
const commissionSheet = commissionWb.Sheets[commissionWb.SheetNames[0]];
const commissionRows = xlsx.utils.sheet_to_json<Record<string, unknown>>(commissionSheet, { defval: '' });
...
const tariffs = commissionRows.flatMap((r) => {
  ...
  return [{ id: slugify(`${category}-${subject}`), category, subject, fboCommissionRate: fbo, fbsCommissionRate: fbs }];
});

fs.writeFileSync(commissionOut, JSON.stringify(tariffs, null, 2), 'utf8');

const warehouseWb = xlsx.readFile(warehousePath);
const sheet = warehouseWb.Sheets['Короба'] ?? warehouseWb.Sheets[warehouseWb.SheetNames[0]];
const rows = xlsx.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '' });

const map: Record<string, { label: string; logistics: number; storage: number; fbs: number | null }> = {};
const aliases: Record<string, string> = {
  'коледино': 'koledino', 'подольск': 'podolsk', 'тула': 'tula', 'казань': 'kazan', 'краснодар': 'krasnodar', 'невинномысск': 'nevinnomyssk', 'екатеринбург': 'ekaterinburg',
};
...
for (const r of rows) {
  ...
  if (!key) continue;
  const logistics = normalizePercent(num(r['Логистика'] ?? r['Коэффициент логистики']));
  const storage = normalizePercent(num(r['Хранение'] ?? r['Коэффициент хранения']));
